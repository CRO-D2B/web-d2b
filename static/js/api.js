...
const getHeaders = (collection, id) => {
  return getRelated(collection + "_headers", collection + "_id", id).then(
    (data) => {
      let idsFilter = ""
...
const getArticulosPorCategoria = (slug) => {
  return getSingleCategoria(slug).then((categoria) => {
    return getRelated("blog_category", "category_id", categoria.id).then(
      (data) => {
        let idsFilter = ""
        data.data.forEach((item) => {
...
const getRelated = (collection, relationship_field, id) => {
  return doFetch(
    "items/" + collection + "?filter[" + relationship_field + "]=" + id
  )
}
